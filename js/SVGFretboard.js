...
fretHeights = 100 / 6;
firstStringFrets = guitarData.filter((o) => o.string == 1);
...
export function getIntervalFill(idx) {
  let colorHash =
    "#FF0000-#E6DF52-#FFDD17-#4699D4-#4699D4-#FFAAAA-#000000-#000000-#bbbbbb-#67AFAD-#8C64AB-#8C64AB".split(
      /-/
    );
  if (idx < 0) {
    idx += 12;
...
export function getFrets(spec) {
  return guitarData.filter((fret) => {
    let chord =
      typeof spec.chord == "string" ? makeChord(spec.chord) : spec.chord;
    return (
      spec.strings.includes(fret.string) &&
      fret.fret >= Math.min(...spec.fretRange) &&
      fret.fret <= Math.max(...spec.fretRange) &&
      chord.abstractNoteValues().includes(fret.noteValue % 12)
    );
  });
}
      ...
      .append(
        guitarData //fixme. pass this in with context or props
          ...
          .map((fret, i, frets) => {
            ...
            let rect = DOM.rect().attr(rectOpts);
            rects.push(rect);
            return rect;
          })
      );
...
*/
  ...
  self.plotFrets = (frets, opts, fretGroup) =>
    frets.forEach((fret) => self.plotFret(fret, opts, fretGroup));

  self.plotFret = function (fret, opts, fretGroup) {
    if (!fretGroup) {
      fretGroup = gFretted;
    }
    let ellipseIdx = self.getEllipseIndex({
      string: fret.string,
      fret: fret.fret,
    });
    ...
    var fillFromInterval =
      fret.interval || fret.interval === 0
        ? getIntervalFill(fret.interval)
        : false;
  ...
  self.bootupFrets = function () {
    ...
    guitarData.forEach((fret) => {
      ...
      var ellipse = DOM.ellipse()
        .attr(opts)
        .on('mouseover', () => self.emit('fret-hover',{
            fret,
            ellipse
        }));
  ...
  self.plotHelper = function ({
    ...
    var frets = getFretsByChromaticHash(hash)
      ...
      .map((fret) => {
        var interval =
          fret.chromaticValue - chordOrScale.rootNote.chromaticValue();
        return {
          ...fret,
          interval,
        };
  ...
  self.plotChordChange = function ({ cursor, strings, fretRange }) {
    let currOpts = {
      chord: cursor.chord,
      fretRange,
      strings,
    ...
    let scout = cursor;
    while (
      scout.chord.rootNote.chromaticValue() ==
      cursor.chord.rootNote.chromaticValue()
    ) {
      scout = scout.next;
    }
    ...
    getFrets(currOpts).forEach((fret) => {
      let idx =
        fret.chromaticValue - currOpts.chord.spec.rootNote.chromaticValue();
      let fill = getIntervalFill(idx);
      let plotOpts = {
        fill
      };
      self.plotFret(fret, plotOpts);
    });
    ...
    getFrets(nextOpts)
      .filter((fret) =>
        nextChordUniqueChromaticValues.contains(fret.chromaticValue)
      )
      .forEach((fret) => {
        let idx =
          fret.chromaticValue - nextOpts.chord.spec.rootNote.chromaticValue();
